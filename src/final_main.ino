...
enum Trigger : uint8_t {
  TRIG_NONE,
  TRIG_AUTH_OK,
  TRIG_TIMER_DONE,  // ← nuevo: solo para expiración de estados temporizados
  TRIG_LOCKOUT,
  TRIG_ENV_ALARM,
  TRIG_INTRUSION
};
...
// UIDs autorizados — 4 bytes cada uno
// Leer el UID de tu tarjeta con el sketch DumpInfo del ejemplo MFRC522
...
bool displayPage = false;  // false=pag1, true=pag2
...
void updateBlinkPattern() {
  ...
  if (now - lastBlink >= interval) {
    ...
    // Buzzer sincronizado con LED solo en BLOCKED
    ...
    case S_ENV_MONITOR:
      {
        ...
        lcd.print(F("  "));  // limpiar caracteres residuales
        break;
      }
    case S_INTRUSION_MONITOR:
      {
        // Alternar entre página 1 y página 2 cada 1 segundo
        ...
        if (!displayPage) {
          ...
          // Página 2: Estado de puerta destacado
...
// Retorna el rol si la tarjeta es válida, 0 si no
uint8_t checkRFID() {
...
void onEnterIntrusionMonitor() {
  ...
  hallDoorOpen = false;
  doorChanged = false;
  lastPageSwitch = millis();  // ← agregar
  displayPage = false;        // ← agregar
 ...
 */
void processInput() {
  // Esperar 1500ms tras cambio de PIN antes de cerrar menú
  if (menuConfirmPending) {
    if (millis() - menuConfirmTime >= 1500) {
      menuConfirmPending = false;
      closeMenu();
    }
    return;  // bloquear teclado mientras se muestra "PIN changed!"
  }
  ...
  if (currentState == S_IDLE) {
    ...
    if (rfidRole > 0) {
      ...
      lastAuthUserIdx = 0xFF;  // RFID no tiene índice EEPROM
      failCount = 0;
      trig = TRIG_AUTH_OK;
      return;  // no procesar teclado este frame
    }
    if (key != NO_KEY) {
      if (menuActive) {
        handleMenuKey(key);
...
void updateState() {
  ...
  if (trig == TRIG_NONE) {
    switch (currentState) {
      case S_OPEN:
        if (elapsed >= T_UNLOCK) trig = TRIG_TIMER_DONE;  // era TRIG_AUTH_OK
        break;
      case S_BLOCKED:
...
void setupFSM() {
  ...
  // --- Desde IDLE ---
  fsm.AddTransition(S_IDLE, S_OPEN, []() {  // auth real
    return trig == TRIG_AUTH_OK;
  });
  fsm.AddTransition(S_IDLE, S_BLOCKED, []() {  // 3 fallos
    return trig == TRIG_LOCKOUT;
  });
  fsm.AddTransition(S_IDLE, S_ENV_MONITOR, []() {  // tecla A
    return trig == TRIG_ENV_ALARM;
  });
  fsm.AddTransition(S_IDLE, S_INTRUSION_MONITOR, []() {  // tecla B
    return trig == TRIG_INTRUSION;
  });

  // --- Timer expirado (solo TRIG_TIMER_DONE) ---
  auto timedDone = []() {
    return trig == TRIG_TIMER_DONE;
  };

  fsm.AddTransition(S_OPEN, S_IDLE, timedDone);  // ← LÍNEA RESTAURADA
...
AsyncTask sensorTask(SENSOR_INTERVAL, true, []() {
  ...
  int ldr = analogRead(PIN_LDR);  // ← invertir LDR para que mayor valor = más luz
 ...
 */
void loop() {
  updateState();   // ← primero: timers y sensores
  processInput();  // ← segundo: teclado puede sobreescribir si es más urgente
  sensorTask.Update();
  fsm.Update();
  trig = TRIG_NONE;
}
