  ...
  switch (currentState) {
    ...
    case S_ALARM:
      if (alarmCount >= 3 && (now - firstAlarmTime) < T_TRIPLE) {
        ...
        // Forzar transición manual sin depender de fsm.Update()
        Serial.println(F("[ALARM] Forcing transition to INTRUSION_MONITOR"));
        onLeaveAlarm();
        currentState = S_INTRUSION_MONITOR;
        onEnterIntrusionMonitor();
        trig = TRIG_NONE;
        // Forzar que fsm también conozca el nuevo estado
