/**
   @file simulator.ino
   @brief Sistema de Control de Acceso y Seguridad — Versión para Simulador.

   @details
   Variante de v4_main.ino para entornos de simulación (Wokwi, Tinkercad, etc.)
   que no soportan StateMachineLib. Reemplaza la FSM basada en librería por una
   implementación manual con el patrón `transitionTo()` / `applyTransition()`.
   El resto del código (pines, sensores, EEPROM, LCD, temporización) es idéntico
   a v4_main.ino.
   ...
   @par Diferencias con v4_main.ino
   - No incluye StateMachineLib
   - Sin objeto `StateMachine fsm` ni variable `trig` / enum `Trigger`
   - FSM manual mediante `transitionTo()` y `applyTransition()` en `loop()`
   - `setupFSM()` solo invoca `onEnterIdle()` — las transiciones se evalúan
     directamente en `updateState()` mediante llamadas a `transitionTo()`
   - Las teclas A/B del teclado disparan `transitionTo()` directamente
   ...
   @par Librerías
   AsyncTaskLib, Keypad 3.1.1, Servo,
              ...
              bool &active, uint8_t &histIdx, char history[4][4])
{
  ...
  for (uint8_t i = 0; i < 4; i++)
    pin[i] = EEPROM.read(addr + OFF_PIN + i);
  pin[4] = '\0';
              ...
              bool active, uint8_t histIdx, const char history[4][4])
{
  uint16_t addr = EEP_USERS_START + idx * EEP_USER_SIZE;
  for (uint8_t i = 0; i < 4; i++)
    EEPROM.update(addr + OFF_PIN + i, pin[i]);
...
void updateBlinkPattern()
{
  ...
  // Si está encendido, esperar blinkOnMs antes de apagar
  // Si está apagado, esperar blinkOffMs antes de encender
  unsigned long interval = ledOn ? blinkOnMs : blinkOffMs; // ← corregido
  ...
  case S_IDLE:
  {
    if (menuActive)
    {
      ...
      if (menuStep == 0)
        ...
        lcd.print(F("Confirm:"));
      for (uint8_t i = 0; i < menuBufLen; i++)
        lcd.print('*');
      for (uint8_t i = menuBufLen; i < PIN_MAX_LEN; i++)
        lcd.print('-');
    }
    else if (pinLen > 0)
    {
      lcd.print(F("IDLE"));
      lcd.setCursor(0, 1);
      for (uint8_t i = 0; i < PIN_MAX_LEN; i++)
        lcd.print(i < pinLen ? '*' : '-');
      lcd.print(F(" #=ok"));
    }
    else
...
// @brief Callbacks de entrada y salida de estado (FSM manual)
// @{
// ============================================================================
...
void onEnterBlocked()
{
  ...
  digitalWrite(PIN_LED_R, HIGH); // ← encender inmediatamente
  blinkActive = true;
  blinkOnMs = BLK_ON;   // 300ms encendido
  blinkOffMs = BLK_OFF; // 700ms apagado
  lastBlink = millis();
  ledOn = true; // ← arranca encendido, coherente con digitalWrite arriba
...
void onEnterAlarm()
{
  ...
  digitalWrite(PIN_LED_R, HIGH); // ← encender inmediatamente
  blinkActive = true;
  blinkOnMs = ALM_ON;   // 100ms encendido
  blinkOffMs = ALM_OFF; // 500ms apagado
  lastBlink = millis();
  ledOn = true; // ← coherente con el HIGH de arriba
  alarmCount++;
  if (alarmCount == 1)
...
/**
   ...
   - En caso exitoso: llama a `transitionTo(S_OPEN)`.
   - En caso fallido: incrementa `failCount`. Al llegar a >= 3: llama a `transitionTo(S_BLOCKED)`.
...
void handlePinEntry(char key)
{
  ...
  { // ← hermano de '#', NO anidado dentro de él
    pinLen = 0;
    memset(pinBuf, 0, sizeof(pinBuf));
  ...
  {
    transitionTo(S_ENV_MONITOR);
  }
  else if (key == 'B')
  {
    transitionTo(S_INTRUSION_MONITOR);
  }
}
/**
...
/**
   @brief Solicita una transición de estado.
   @details Se evalúa al final de cada loop(). Llama onLeave del estado
   actual y onEnter del nuevo estado. Idempotente si ya hay una
   transición pendiente (la primera gana).
*/
void transitionTo(State next)
{
  if (stateChangePending)
    return; // ya hay una transición en cola
  pendingState = next;
  stateChangePending = true;
}

/**
   @brief Ejecuta la transición pendiente (si existe).
   @details Debe llamarse al final de loop(), después de updateState().
*/
...
/**
   ...
   2. **Temporización de estado**: Para cada estado temporizado (OPEN, BLOCKED, etc.),
      verifica si el tiempo transcurrido supera la duración y llama a
      `transitionTo(S_IDLE)` para encolar una transición de regreso.
...
void updateState()
{
  ...
  if (!stateChangePending)
  {
    ...
    case S_OPEN:
      ...
      break;
    case S_BLOCKED:
      if (elapsed >= T_LOCKOUT)
        transitionTo(S_IDLE);
      break;
    case S_ENV_MONITOR:
      if ((lightLevel > 0 && lightLevel < LIGHT_MIN))
        transitionTo(S_IDLE);
      break;
    case S_INTRUSION_MONITOR:
      if (elapsed >= T_INTRUSION)
        transitionTo(S_IDLE);
      break;
    case S_ALARM:
      if (elapsed >= T_ALARM)
  ...
  case S_ENV_MONITOR:
         ...
         (lightLevel > 900)))
    {
      ...
      transitionTo(S_ALARM);
    }
    break;

  case S_INTRUSION_MONITOR:
    if (!stateChangePending && (hallDoorOpen || micVal > SOUND_HIGH))
    {
      if (hallDoorOpen)
      {
        doorChanged = false;
      ...
      if (micVal > SOUND_HIGH)
        Serial.println(F("[INTRUSION] Sound detected!"));
      transitionTo(S_ALARM);
    }
    break;
  ...
  case S_ALARM:
    if (alarmCount >= 3 && (now - firstAlarmTime) < T_TRIPLE)
    {
      ...
      alarmCount = 0;
      stateEntryTime = now; // reiniciar timer — permanecer en alarma
    }
    // la transición por tiempo ya está en el bloque de temporización de arriba
    break;
...
// @brief Inicialización de la FSM manual para simulador
// @{
// ============================================================================

/**
   @brief Inicializa el estado inicial de la FSM manual.
   @details
   Versión para simulador: como no se utiliza StateMachineLib, no hay
   transiciones que cablear. Simplemente se ejecuta el manejador de entrada
   del estado S_IDLE para dejar el sistema en reposo.

   Las transiciones se evalúan directamente en `updateState()` mediante
   llamadas a `transitionTo()`, y se ejecutan al final de `loop()` con
   `applyTransition()`.

   @par Tabla de transiciones (implementada en updateState())
   @code{.txt}
   S_IDLE --[auth ok]--> S_OPEN
   S_IDLE --[3 fallos]--> S_BLOCKED
   S_OPEN --[timer 2s]--> S_IDLE
   S_BLOCKED --[timer 5s]--> S_IDLE
   S_ENV_MONITOR --[umbral temp/light]--> S_ALARM
   S_ENV_MONITOR --[sin evento 4s]--> S_IDLE
   S_INTRUSION_MONITOR --[hall/mic]--> S_ALARM
   S_INTRUSION_MONITOR --[timer 2s]--> S_IDLE
   S_ALARM --[timer 2s]--> S_INTRUSION_MONITOR
   @endcode
...
void setupFSM()
{
  // Sin librería: solo disparar el enter del estado inicial
  onEnterIdle();
}
...
AsyncTask sensorTask(SENSOR_INTERVAL, true, []()
  ...
  int ldr = 1023 - analogRead(PIN_LDR);   // ← invertir LDR también
...
/**
   @brief Bucle principal de Arduino (versión simulador).
   @details
   ...
   2. `updateState()`: Procesar interrupciones, temporizadores, sensores, parpadeo, LCD.
      Aquí se evalúan las condiciones de transición mediante `transitionTo()`.
   3. `sensorTask.Update()`: Ejecutar tareas periódicas de promediado de sensores.
   4. `applyTransition()`: Ejecutar la transición de estado pendiente (si existe),
      invocando los callbacks onLeave/onEnter correspondientes.

   @note
   A diferencia de v4_main.ino, no se utiliza `fsm.Update()` de StateMachineLib.
   En su lugar, `applyTransition()` ejecuta manualmente los callbacks de cambio
   de estado. Toda la temporización usa `millis()` para operación no bloqueante.
   ...
   - Transiciones de estado: evaluadas en updateState(), ejecutadas en applyTransition().
